...
std::vector<int> dijkstraParallel(const std::vector<int>& graph, int start, int end) {
    ...
    //     tbb::parallel_for(tbb::blocked_range<int>(0, points_count, grainsize),
    //         [&](const tbb::blocked_range<int>& range) {
    //     for (int k = range.begin(); k < range.end(); k++) {
    //         if (!processed[k] && points_len[min_point] != max_weight &&
    //             graph[min_point*points_count + k] &&
    //             points_len[min_point] +
    //             graph[min_point*points_count + k] < points_len[k]) {
    //             points_len[k] = points_len[min_point] +
    //             graph[min_point*points_count + k];
    //         }
    //     }
    //     });
